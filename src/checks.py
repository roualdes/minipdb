...
def model_name_check(toml, tomlfile):
    if "model_name" not in toml:
        sys.exit(
            f"No model name specified, please specify model_name in {tomlfile} and add again."
        )
    else:
        if not toml["model_name"][0].isupper():
            raise ValueError(
                error_message(
                    "model_name", tomlfile, "begin with an upper case ACII character"
                )
            )
        ...
        if not set(toml["model_name"]) <= allowed_chars:
            raise ValueError(
                error_message(
                    "model_name", tomlfile, "contain only ACII characters, digits, _-"
                )
            )
...
def stan_file_check(toml, tomlfile):
    if "stan_file" not in toml:
        sys.exit(
            f"No Stan file specified, please specify stan_file in {tomlfile} and add again."
        )
    else:
...
def json_data_check(toml, tomlfile):
    if "json_data" not in toml:
        sys.exit(
            "No JSON data specified, please specify json_data in {tomlfile} and add again."
        )
    else:
...
def iter_sampling_check(toml, tomlfile):
    if "iter_sampling" not in toml["meta"]:
        ...
        if (
            not 2_000 <= toml["meta"]["iter_sampling"]
            or not toml["meta"]["iter_sampling"] <= 1_000_000
        ):
            raise ValueError(
                error_message(
                    "iter_sampling", tomlfile, "be an integer in 2_000:1_000_000."
                )
            )
...
def iter_warmup_check(toml, tomlfile):
    if "iter_warmup" not in toml["meta"]:
        ...
        if (
            not 2_000 <= toml["meta"]["iter_warmup"]
            or not toml["meta"]["iter_warmup"] <= 1_000_000
        ):
            raise ValueError(
                error_message(
                    "iter_sampling", tomlfile, "be an integer in 2_000:1_000_000."
...
def chains_check(toml, tomlfile):
    if "chains" not in toml["meta"]:
        ...
        if not 1 <= toml["meta"]["chains"]:
            raise ValueError(
                error_message("chains", tomlfile, "be an integer greater than zero.")
            )
...
def parallel_chains_check(toml, tomlfile):
    if "parallel_chains" not in toml["meta"]:
        ...
        if type(toml["meta"]["parallel_chains"]) is not int:
            raise TypeError(
                error_message("parallel_chains", tomlfile, "be an integer.")
            )

        if (
            not 1 <= toml["meta"]["parallel_chains"]
            or not toml["meta"]["parallel_chains"] <= toml["chains"]
        ):
            raise ValueError(
                error_message(
                    "parallel_chains", tomlfile, f'be an integer in 1:{toml["chains"]}'
                )
            )
...
def thin_check(toml, tomlfile):
    if "thin" not in toml["meta"]:
        ...
        if (
            not 1 <= toml["meta"]["thin"]
            or not toml["meta"]["thin"] <= toml["iter_sampling"]
        ):
            raise ValueError(
                error_message(
                    "thin",
                    tomlfile,
                    f'be an integer between 1 and {toml["iter_sampling"]}.',
                )
            )
...
def seed_check(toml, tomlfile):
    if "seed" not in toml["meta"]:
        ...
        if not 0 <= toml["meta"]["seed"] or not toml["meta"]["seed"] <= 4_294_967_295:
            raise ValueError(
                error_message("seed", tomlfile, "be an integer in 0:4_294_967_295.")
            )
...
def adapt_delta_check(toml, tomlfile):
    if "adapt_delta" not in toml["meta"]:
        ...
        if not 0 < toml["meta"]["adapt_delta"] or not toml["meta"]["adapt_delta"] < 1:
            raise ValueError(
                error_message("adapt_delta", tomlfile, "be a float in (0, 1).")
            )
...
def max_treedepth_check(toml, tomlfile):
    if "max_treedepth" not in toml["meta"]:
        ...
        if (
            not 1 <= toml["meta"]["max_treedepth"]
            or not toml["meta"]["max_treedepth"] <= 20
        ):
            raise ValueError(
                error_message("max_treedepth", tomlfile, "be an integer in 1:20.")
            )
...
def sig_figs_check(toml, tomlfile):
    if "sig_figs" not in toml["meta"]:
        ...
        if not 1 <= toml["meta"]["sig_figs"] or not toml["meta"]["sig_figs"] <= 18:
            raise ValueError(
                error_message("sig_figs", tomlfile, "be an integer in 1:18.")
            )
