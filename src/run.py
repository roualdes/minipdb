...
def run_model(model_name: str, models_dir: str = "", database_path: str = ""):
    ...
    db.execute(f'UPDATE Program SET "last_run" = "{t}" WHERE "model_name" = "{model_name}"')
    db.commit()
...
def run(args: dict):
    ...
    if sum([exists_in_program, exists_in_meta]) == 0:
        sys.exit(f'Program {model_name} does not exists in database, please init first.')
...
def run_all(args: dict):
    ...
    with Pool(args['cpus']) as p:
        f = functools.partial(run_model, models_dir = model_folder, database_path = database)
        p.map(f, model_names)
